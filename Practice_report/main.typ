...
#show: project.with(
  ...
  task_type: "Курсовой проект по курсу",
  task_name: "\"Метод конечных элементов\"",
  students: (
    "Данченко Иван",
  ),
  //variant: "",
...
_1. количество разбиений по x
2-n. начало по х | конец по х | кол-во шагов | коэф. разрядки
...
14-n. значение сигмы | индекс начала зоны по x | индекс конца зоны по x | индекс начала зоны по y | индекс конца зоны по y | индекс начала зоны по z | индекс конца зоны по z_
...
$ #A()\("r"\) = frac(mu_0, 4 pi)  limits(integral)_(Omega) frac(#J()\("r"^'\) times ("r" - "r"^'), bar.v.double "r" - "r"^' bar.v.double) "dr"^' $
...
$ #A() = sum_(i=1)^n frac(mu_0, 4 pi)  frac(#J() dot "dh", sqrt(("x" - "x"^')^2 + ("y" - "y"^')^2 + ("z" - "z"^')^2)), $
...
#figure([#image("pics/1.png", width: 100%)])

Также, чем дальше ребро находится от генератора, тем меньшее значение поля А имеет ребро. В таблице ниже представлен пример убывания поля от центра генератора. Для этого фиксируются значения по y и z и изменяется только значение по x.

#let results = csv("csv/Ax.csv")
...
== Расчёт вектора индукции магнитного поля $#B()$

Вектор индукции магнитного поля можно выразить через $#A()$:

$ #B() = "rot" #A() $

Для рассчёта применяется метод аппроксимации производной по базисным функциям, которые принимают вид:

$ #B() = sum_(i=0)^n "rot"limits(psi)^(->) dot A_i $

== Расчёт вектора напряжённости электрического поля $#E()$

Вектор вектор напряжённости электрического поля можно выразить через $#A()$:

$ #E() =  frac("d"#A(), "dt") $

Для рассчёта применяется метод аппроксимации по временной сетке, которые принимают вид:

$ #E() = frac(#A()\("t"\) - #A()\("t" - tau\), "t" - tau) $
